...
Java_org_kde_kdeconnect_Helpers_OpdlKernelBridge_nativeHasKernelDevice(JNIEnv *env, jobject thiz) {
    (void)env;
    (void)thiz;
...
JNIEXPORT jboolean JNICALL
Java_org_kde_kdeconnect_Helpers_OpdlKernelBridge_nativeSendPayloadFile(
        JNIEnv *env, jobject thiz, jstring device_id, jstring payload_path, jlong payload_size) {
    (void)thiz;
    (void)device_id;
    ...
    while (total_written < payload_size) {
        ...
        while (offset < read_count) {
            ssize_t written = write(opdl_fd, buffer + offset, (size_t)(read_count - offset));
            if (written < 0) {
...
Java_org_kde_kdeconnect_Helpers_OpdlKernelBridge_nativeReceivePayloadFile(
        JNIEnv *env, jobject thiz, jstring device_id, jstring destination_path, jlong expected_size) {
    (void)thiz;
    (void)device_id;
    ...
    while (total_read < expected_size) {
        ...
        ssize_t offset = 0;
        while (offset < read_count) {
            ssize_t written = write(dst_fd, buffer + offset, (size_t)(read_count - offset));
            if (written < 0) {
