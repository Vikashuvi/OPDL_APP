...
import org.opdl.transfer.Plugins.NotificationsPlugin.NotificationReceiver
import org.opdl.transfer.Plugins.SystemVolumePlugin.SystemVolumePlugin
import org.opdl.transfer.Plugins.SystemVolumePlugin.SystemVolumeProvider
...
class MprisMediaSession : OnSharedPreferenceChangeListener, NotificationReceiver.NotificationListener,
    ...
    private fun findPlayer(): Pair<Device, MprisPlayer>? {
        val currentDevice = if (notificationDeviceId != null && mprisDevices.contains(notificationDeviceId)) {
            OpdlTransfer.getInstance().getDevice(notificationDeviceId)
        } else {
            null
        }
        ...
        for (otherDevice in OpdlTransfer.getInstance().devices.values) {
    ...
    private fun updateRemoteDeviceVolumeControl() {
        val plugin = OpdlTransfer.getInstance().getDevicePlugin(notificationDeviceId, SystemVolumePlugin::class.java)
            ?: return
    ...
    private fun updateMediaNotification() {
        ...
        val device = OpdlTransfer.getInstance().getDevice(notificationDeviceId)
        if (device == null) {
            closeMediaNotification()
            return
        }
