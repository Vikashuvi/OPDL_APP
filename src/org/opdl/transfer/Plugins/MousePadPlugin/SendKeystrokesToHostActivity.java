...
import org.opdl.transfer.Helpers.SafeTextChecker;
...
import org.opdl.transfer_tp.databinding.ActivitySendkeystrokesBinding;
...
public class SendKeystrokesToHostActivity extends BaseActivity<ActivitySendkeystrokesBinding> {
    ...
    protected void onStart() {
        ...
        if (!prefs.getBoolean(getString(R.string.pref_sendkeystrokes_enabled), true)) {
            ...
            if ("text/x-keystrokes".equals(type)) {
                ...
                if (contentIsOkay) {
                    List<Device> reachableDevices = OpdlTransfer.getInstance().getDevices().values().stream()
                ...
                OpdlTransfer.getInstance().addDeviceListChangedCallback("SendKeystrokesToHostActivity", () -> runOnUiThread(this::updateDeviceList));
    ...
    protected void onStop() {
        OpdlTransfer.getInstance().removeDeviceListChangedCallback("SendKeystrokesToHostActivity");
        super.onStop();
    }
    ...
    private void sendKeys(Device deviceId) {
        ...
        if (getBinding().textToSend.getText() != null && (toSend = getBinding().textToSend.getText().toString().trim()).length() > 0) {
            MousePadPlugin plugin = OpdlTransfer.getInstance().getDevicePlugin(deviceId.getDeviceId(), MousePadPlugin.class);
            if (plugin == null) {
                finish();
                return;
            }
