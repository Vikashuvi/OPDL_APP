...
class ReceiveNotification {
    ...
    public void setURI(Uri destinationUri, String mimeType, String filename) {
        /*
         * We only support file URIs (because sending a content uri to another app does
         * not work for security reasons).
         * In effect, that means only the default download folder currently works.
         *
         * TODO: implement our own content provider (instead of support-v4's
         * FileProvider). It should:
        ...
        if (mimeType.startsWith("image/")) {
            ...
            try (InputStream decodeBoundsInputStream = device.getContext().getContentResolver()
                    .openInputStream(destinationUri);
                    InputStream decodeInputStream = device.getContext().getContentResolver()
                            .openInputStream(destinationUri)) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N && "file".equals(destinationUri.getScheme())) {
            ...
            Uri contentUri = FileProvider.getUriForFile(device.getContext(),
        ...
        PendingIntent sharePendingIntent = PendingIntent.getActivity(device.getContext(),
