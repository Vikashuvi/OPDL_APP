...
import org.opdl.transfer.Helpers.MediaStoreHelper;
...
public class CompositeReceiveFileJob extends BackgroundJob<Device, Void> {
    ...
    private long receiveFile(InputStream input, OutputStream output) throws IOException {
        ...
        while ((count = input.read(data)) >= 0 && !isCancelled()) {
            ...
            synchronized (lock) {
                progressPercentage = totalPayloadSize > 0 ? (totalReceived * 100 / totalPayloadSize) : 100;
            }
    ...
    private String getMeasuredSpeed(long bytesReceivedSoFar) {
        if (startTimeMillis <= 0) {
            return "0.0 MB/s";
        }
        long elapsedMillis = Math.max(1, System.currentTimeMillis() - startTimeMillis);
        double mbPerSecond = (bytesReceivedSoFar / (1024.0 * 1024.0)) / (elapsedMillis / 1000.0);
        return String.format("%.1f MB/s", mbPerSecond);
    }

    private String getDetailedMetrics(String speed, long bytesReceivedSoFar) {
        double receivedMB = bytesReceivedSoFar / (1024.0 * 1024.0);
        ...
        synchronized (lock) {
            ...
            String message = getDevice().getContext().getResources()
                    .getQuantityString(R.plurals.incoming_files_text, totalNumFiles, currentFileName, currentFileNum,
                            totalNumFiles);
            String detailedMetrics = getDetailedMetrics(speed, totalReceived);
            receiveNotification.setProgress(progress, message, speed, detailedMetrics);
        }
        receiveNotification.show();
    }
    ...
    private void openFile(DocumentFile fileDocument) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            ...
            Uri contentUri = FileProvider.getUriForFile(getDevice().getContext(), "org.opdl.transfer_tp.fileprovider",
                    file);
