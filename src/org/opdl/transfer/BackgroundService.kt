...
import org.opdl.transfer.Backends.BaseLinkProvider.ConnectionReceiver
import org.opdl.transfer.Backends.BluetoothBackend.BluetoothLinkProvider
import org.opdl.transfer.Backends.LanBackend.LanLinkProvider
import org.opdl.transfer.Helpers.NotificationHelper
import org.opdl.transfer.Plugins.ClipboardPlugin.ClipboardFloatingActivity
import org.opdl.transfer.Plugins.RunCommandPlugin.RunCommandActivity
 ...
 * - Holds references to the active LinkProviders, but doesn't handle the DeviceLink those create (the OpdlTransfer class does that).
 * - Listens for network connectivity changes and tells the LinkProviders to re-check for devices.
 * It can be started by the OpdlTransferBroadcastReceiver on some events or when the MainActivity is launched.
 */
class BackgroundService : Service() {
    private lateinit var applicationInstance: OpdlTransfer
    ...
    override fun onCreate() {
        super.onCreate()
        Log.d("OpdlTransfer/BgService", "onCreate")
        this.applicationInstance = OpdlTransfer.getInstance()
        instance = this

        OpdlTransfer.getInstance().addDeviceListChangedCallback("BackgroundService", this::updateForegroundNotification)
        ...
        registerReceiver(OpdlTransferBroadcastReceiver(), filter)
    ...
    private fun createForegroundNotification(): Notification {
        ...
        if (connectedDevices.isEmpty()) {
            ...
            if (connectedDeviceIds.size == 1) {
                ...
                val device = OpdlTransfer.getInstance().getDevice(deviceId)
                if (device != null) {
    ...
    override fun onDestroy() {
        Log.d("OpdlTransfer/BgService", "onDestroy")
        initialized = false
        for (linkProvider in linkProviders) {
            linkProvider.onStop()
        }
        OpdlTransfer.getInstance().removeDeviceListChangedCallback("BackgroundService")
        super.onDestroy()
    }
