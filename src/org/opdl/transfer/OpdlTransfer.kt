...
import org.opdl.transfer.Helpers.LifecycleHelper
import org.opdl.transfer.Helpers.NotificationHelper
import org.opdl.transfer.Helpers.SecurityHelpers.RsaHelper
import org.opdl.transfer.Helpers.SecurityHelpers.SslHelper
import org.opdl.transfer.Helpers.ThreadHelper
import org.opdl.transfer.PairingHandler.PairingCallback
 ...
 */
class OpdlTransfer : Application() {
    ...
    override fun onCreate() {
        ...
        _instance = this
        setupSL4JLogging()
        Log.d("OpdlTransfer/Application", "onCreate")
    ...
    override fun onTerminate() {
        Log.d("OpdlTransfer/Application", "onTerminate")
        super.onTerminate()
    }
    ...
    private fun loadRememberedDevicesFromSettings() {
        ...
        trustedDevices.asSequence()
            .onEach { Log.d("OpdlTransfer", "Loading device $it") }
    ...
    fun removeRememberedDevices() {
        ...
        trustedDevices.filter { preferences.getBoolean(it, false) }
            .forEach {
                Log.d("OpdlTransfer", "Removing devices: $it")
                preferences.edit { remove(it) }
            }
    }
    ...
    val connectionListener: ConnectionReceiver = object : ConnectionReceiver {
        ...
        override fun onConnectionReceived(link: BaseLink) {
            ...
            if (device != null) {
                ...
                device = Device(this@OpdlTransfer, link)
        ...
        override fun onDeviceInfoUpdated(deviceInfo: DeviceInfo) {
            ...
            if (device == null) {
                Log.e("OpdlTransfer", "onDeviceInfoUpdated for an unknown device")
                return
            }
    ...
    fun scheduleForDeletion(device: Device) {
        ...
        Log.i("OpdlTransfer", "Scheduled for deletion: $device, paired: ${device.isPaired}, reachable: ${device.isReachable}")
        ThreadHelper.execute {
            try {Thread.sleep(1000)} catch (_: InterruptedException) { }
            if (device.isReachable) {
                Log.i("OpdlTransfer", "Not deleting device since it's reachable again: $device")
                return@execute
            }
            if (device.isPaired) {
                Log.i("OpdlTransfer", "Not deleting device since it's still paired: $device")
                return@execute
            }
            Log.i("OpdlTransfer", "Deleting unpaired and unreachable device: $device")
    ...
    companion object {
        @JvmStatic
        private lateinit var _instance: OpdlTransfer

        @JvmStatic
        fun getInstance(): OpdlTransfer = _instance
    }
}
