package org.opdl.transfer.Plugin
...
import org.opdl.transfer.Plugins.PingPlugin.PingPlugin
...
class PingPluginTest {
    ...
    fun startPlugin() {
        executeWithMocks { device, plugin, notificationManager ->
            ...
            verify(exactly = 1) { device.sendPacket(match { np -> np.type == "opdltransfer.ping" && np.payload == null }) }
        }
    }
    ...
    fun wrongPacketType() {
        executeWithMocks { device, plugin, notificationManager ->
            val np = NetworkPacket("opdltransfer.wrench")
    ...
    fun sendsNotificationNoMessage() {
        executeWithMocks { device, plugin, notificationManager ->
            val np = NetworkPacket("opdltransfer.ping")
